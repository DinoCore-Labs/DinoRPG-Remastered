...
export async function gatherWithDinozHandler(
	...
	const ingredientQuantityBefore = new Map<number, number>();
	for (const ingredient of returnGrid.rewards.ingredients) {
		const rewardQuantity = getIngredientRewardQuantity(ingredient.quantity);
		let ingredientToReward = user.ingredients.find(entry => entry.ingredientId === ingredient.ingredientId);
		const quantityBefore = ingredientQuantityBefore.get(ingredient.ingredientId) ?? ingredientToReward?.quantity ?? 0;
		ingredientQuantityBefore.set(ingredient.ingredientId, quantityBefore);
		let currentQuantity = ingredientToReward?.quantity ?? 0;
		for (let index = 0; index < rewardQuantity; index += 1) {
			...
			ingredientToReward = await addIngredientToInventory(user.id, ingredient.ingredientId, 1);
			user.ingredients.push(ingredientToReward);
			currentQuantity = ingredientToReward.quantity;
		}
		const isMaxQuantity = currentQuantity >= ingredient.maxQuantity;
